      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 mb-3">
            <div className="w-9 h-9 rounded-xl bg-violet-600 grid place-items-center text-white">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
            </div>
            <span className="text-2xl font-bold text-gray-900">Co</span>
          </div>
          <p className="text-gray-500 text-sm">Real-time collaborative editing</p>
        </div>
        ...
        <div className="bg-white border border-violet-100 rounded-2xl p-8 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-semibold text-sm disabled:opacity-60 transition">
        ...
        <div className="mt-4 text-center bg-white border border-violet-100 rounded-2xl p-4 text-sm">
          {mode === 'login'
            ? <>New here? <button onClick={() => setMode('signup')} className="text-violet-600 font-semibold hover:underline">Create account</button></>
            : <>Already have an account? <button onClick={() => setMode('login')} className="text-violet-600 font-semibold hover:underline">Sign in</button></>}
        </div>
      </div>
    </div>
  );
